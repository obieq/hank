...
app.controller('SuiteController',['$scope', '$stateParams', '$state', 'CrudService', 'ngAppSettings', 'CommonUiService', 'CommonDataProvider',
  function ($scope, $stateParams, $state, crudService, ngAppSettings, commonUi, dataProvider) {
    ...
    $scope.getSuiteById = function(){
      crudService.getById(ngAppSettings.SuiteUrl, $stateParams.SuiteId).then(function(response){
    ...
    $scope.addSuite = function(){
      ...
      crudService.add(ngAppSettings.SuiteUrl, $scope.Suite).then(function(response){
        $scope.addSuiteTestLinks(response.Item.Id, true);
      },function(response){ commonUi.showErrorPopup(response); });
    };

    $scope.updateSuite = function(){
      crudService.update(ngAppSettings.SuiteUrl, $scope.Suite).then(function(response){
    ...
    $scope.addSuiteTestLinks = function(suiteId, doRedirect){
      ...
      crudService.add(ngAppSettings.SuiteTestMapUrl.format(suiteId), mapData).then(function(){
        if(doRedirect) {
    ...
    $scope.getSuiteTestLinks = function(suiteId){
      crudService.getAll(ngAppSettings.SuiteTestMapUrl.format(suiteId)).then(function(response){
    ...
    $scope.bindTest = function(suiteData){
      if(suiteData.WebsiteId == 0) {
        ...
        crudService.getAll(ngAppSettings.WebSiteTestCasesUrl.format(suiteData.WebsiteId)).then(function(response) {
